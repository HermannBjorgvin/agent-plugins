...
#                                             bare session names mean tmux (legacy)
...
# a dead pane or gone entirely. The prompt defaults to "continue"; give --prompt for a new
# assignment. The original task is never replayed. Harness: --agent, else the session's
# @player-agent tag, else the harness recorded in the worktree, else Claude --continue and,
# only when Claude reports "No conversation found to continue", Codex (the newest recorded
# Codex conversation whose cwd is this worktree). Any other failure stops with a dead pane
# for inspection; nothing starts a fresh conversation silently. --model/--effort are applied
# on resume only when given; otherwise the CLI's restored/configured settings apply.
#
...
# launcher prefixes it with the player invocation: $orchestra:player for Codex, and for Claude the
...
while [ $# -gt 0 ]; do case "$1" in
  ...
  --dry-run) DRY=1;; --resume) RESUME=1;; -h|--help) sed -n '2,40p' "$0"; exit 0;;
...
if [ -z "$PROMPT" ]; then
  [ $RESUME = 1 ] && PROMPT="continue" || { echo "spawn.sh: task prompt is required (--prompt or --prompt-file)" >&2; exit 2; }
fi
...
PLAYER_SKILL="$(claude_player_invocation)"
root="$(repo_root)"
...
if t has-session -t "=$name" 2>/dev/null; then
  ...
  elif [ "$(t show-options -v -t "$tt" @player-launching 2>/dev/null)" = 1 ]; then EXISTING=placeholder
...
if [ $RESUME = 1 ] && [ -z "$HARNESS" ]; then
  [ "$EXISTING" = none ] || HARNESS="$(t show-options -v -t "$tt" @player-agent 2>/dev/null || true)"
  if [ -z "$HARNESS" ]; then
    recorded="$(git -C "$root/$dir" rev-parse --absolute-git-dir 2>/dev/null)/player-agent"
    [ -s "$recorded" ] && HARNESS="$(head -n1 "$recorded")"
  fi
...
# Persist the reporting binding (report.sh prefers it over environment) and the task body.
(cd "$root/$dir" && bash "$REPORT_SCRIPT" --orchestrator "$ORCH" --socket "$ORCH_SOCK") || exit 1
gitdir="$(git -C "$root/$dir" rev-parse --absolute-git-dir)" || exit 1
pfile="$gitdir/player-prompt"
tmpf="$(mktemp "$pfile.XXXXXX")" && printf '%s' "$PROMPT" > "$tmpf" && mv "$tmpf" "$pfile" || { echo "spawn.sh: cannot write $pfile" >&2; exit 1; }
...
t set-option -t "$tt" @player-launching 1
t set-option -t "$tt" status off
t set-option -t "$tt" remain-on-exit on
case "$HARNESS" in auto|custom) ;; *) t set-option -t "$tt" @player-agent "$HARNESS";; esac
if ! t respawn-pane -k -t "$tt" -c "$root/$dir" \
  -e "PATH=$PATH" -e "HOME=$HOME" \
  -e "PLAYER_MODE=$MODE" -e "PLAYER_HARNESS=$HARNESS" -e "PLAYER_MODEL=$MODEL" -e "PLAYER_EFFORT=$EFFORT" \
  -e "PLAYER_PERM=$PERM" -e "PLAYER_CMD=$CMD" -e "PLAYER_PROMPT_FILE=$pfile" -e "PLAYER_CLAUDE_SKILL=$PLAYER_SKILL" \
  -e "ORCHESTRATOR_TARGET=$ORCH" -e "ORCHESTRATOR_SESSION=${ORCH#tmux:}" \
  -e "ORCHESTRATOR_TMUX_SOCKET=$ORCH_SOCK" -e "PLAYER_NAME=$(session_name_for_branch "$BRANCH")" \
...
t set-option -u -t "$tt" @player-launching
echo "started   $name"
