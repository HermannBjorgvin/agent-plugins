...
#
# A player is a session tagged @orchestra-spawner (any value) and @orchestra-session-type
...
# A row is in scope when its tags say player and, unless --all, its repo tag equals this repo.
in_scope() { [ -n "$1" ] && [ "$2" = "$SESSION_TYPE_WORKTREE" ] && { [ $ALL = 1 ] || [ "$3" = "$scope" ]; }; }
declare -A before
if [ "$SAMPLE" -gt 0 ]; then
...
FORMAT="#{session_name}${TAB}#{pane_dead}${TAB}#{pane_current_command}${TAB}#{window_activity}${TAB}#{pane_current_path}${TAB}#{$TAG_AGENT}${TAB}#{$TAG_ORCHESTRATOR}${TAB}#{$TAG_LAST_REPORT}${TAB}#{$TAG_REPO}${TAB}#{$TAG_BRANCH}${TAB}#{$TAG_SPAWNER}${TAB}#{$TAG_SESSION_TYPE}${TAB}#{pane_title}"
split_tabs() {
  local line="$1"; F=()
...
while IFS= read -r line; do
  ...
  name="${1:-}"; dead="${2:-}"; cmd="${3:-}"; activity="${4:-}"; path="${5:-}"; agent="${6:-}"; orch="${7:-}"; last="${8:-}"; tag_repo="${9:-}"; branch="${10:-}"; spawner="${11:-}"; type="${12:-}"
